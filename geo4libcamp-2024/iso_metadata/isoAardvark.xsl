<xsl:stylesheet xmlns="http://www.loc.gov/mods/v3" xmlns:gco="http://www.isotc211.org/2005/gco"
  ...
  <xsl:template match="/">
    ...
    <!-- 07. Provider. Recommended. Not Repeatable. See notes for institution variable -->
    ...
    <xsl:if
      test="gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/resourceConstraints/MD_LegalConstraints/gmd:otherConstraints/gco:CharacterString">
      <xsl:text>"dct_rights_sm": ["</xsl:text>
      <xsl:value-of select="gmd:MD_Metadata/gmd:identificationInfo/gmd:MD_DataIdentification/resourceConstraints/MD_LegalConstraints/gmd:otherConstraints/gco:CharacterString"/>
      <xsl:text>"],</xsl:text>
    </xsl:if>
